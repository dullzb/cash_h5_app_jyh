    ...
    <div class="content">
      ...
      <div class="tv7" @click="selectClick(1)">
        <div class="tv8">
          ...
          <input class="tv10" readonly unselectable="on" disabled="disabled" v-model="formData.educationName"
                 placeholder="请选择"/>
      ...
      <div class="tv7" @click="selectClick(2)">
        <div class="tv8">
          ...
          <input class="tv10" readonly unselectable="on" disabled="disabled" v-model="formData.liveTimeName"
                 placeholder="请选择"/>
          <img class="iv1" src="../../assets/more@3x.png"/>
        </div>
      </div>
      <div class="tv7" @click="selectClick(3)">
        <div class="tv8">
          ...
          <input class="tv10" readonly unselectable="on" disabled="disabled" v-model="formData.isMarryName"
                 placeholder="请选择"/>
          <img class="iv1" src="../../assets/more@3x.png"/>
        </div>
      </div>
      <div class="tv7" @click="selectClick(4)">
        <div class="tv8">
          ...
          <input class="tv10" readonly unselectable="on" disabled="disabled" v-model="formData.jobName"
                 placeholder="请选择"/>
          <img class="iv1" src="../../assets/more@3x.png"/>
        </div>
      </div>
      ...
      <div class="tv7" @click="selectClick(5)">
        <div class="tv8">
          ...
          <input class="tv10" readonly unselectable="on" disabled="disabled" v-model="formData.linealRelationName"
                 placeholder="请选择"/>
          <img class="iv1" src="../../assets/more@3x.png"/>
        </div>
      </div>
      ...
      <div class="tv7" @click="selectClick(6)">
        <div class="tv8">
          <div class="tv9">与本人关系</div>
          <input class="tv10" readonly unselectable="on" disabled="disabled" v-model="formData.linealRelationName2"
                 placeholder="请选择"/>
          <img class="iv1" src="../../assets/more@3x.png"/>
        </div>
      </div>
...
<script>
  ...
  export default {
    ...
    data() {
      return {
        ...
        formData: {
          ...
          'educationName': '', //'： 博士 硕士 本科 大专 中专 高中 初中 初中以下',
          'provinceNow': '', // '省',
          'cityNow': '', // '市',
          'districtNow': '', // '区',
          'addressNow': '', // '现居住地',
          'liveTime': '', // '居住时长:半年以内 半年到一年 一年以上 单位年 半年0.5 一年1',
          'liveTimeName': '', // '居住时长:半年以内 半年到一年 一年以上 单位年 半年0.5 一年1',
          'isMarry': '', // '未婚 已婚未育 已婚已育 离异 0 1 2 3',
          'isMarryName': '', // '未婚 已婚未育 已婚已育 离异 0 1 2 3',
          'job': '', // '上班族 自由职业 学生',
          'jobName': '', // '上班族 自由职业 学生',
          'companyName': '', // '工作单位',
          'companyAddress': '', // '公司地址',
          'linealRelation': '', //'父亲 母亲 兄弟 姐妹 儿子 女儿 配偶 1 2 3 4 5',
          'linealRelationName': '', //'父亲 母亲 兄弟 姐妹 儿子 女儿 配偶 1 2 3 4 5',
          'linealMail': '', //'直系联系方式',
          'linealRelation2': '', //'父亲 母亲 兄弟 姐妹 儿子 女儿 配偶 1 2 3 4 5',
          'linealRelationName2': '', //'父亲 母亲 兄弟 姐妹 儿子 女儿 配偶 1 2 3 4 5',
          'linealMail2': '', //'紧急联系方式',
          'userMails': []
        },
        selectList: [], //当前筛选数据
    ...
    created() {
      window.setTelList = this.setTelList;
      ...
      if (isAndroid) {
        ...
        // window.webkit.messageHandlers.getAddressBook.postMessage()

        window.location.href = 'cashloans://getAddressBook';
      }
    }
    ,
    mounted() {
    ...
    methods: {
      ...
      selectClick(type) {
        ...
        if (1 == type) {
          ...
          this.selectList = this.selectAllList.relation2;
        }
      },
      //现居地址点击事件
      areaClick() {
      ...
      systemConfigList() {
        ...
        Base.requestPos(params1, '/systemConfig/list').then((response) => {
          if (response.data.status == 'SUCCESS') {
            ...
            for (let i = 0; i < response.data.t.list.relation2.length; i++) {
              response.data.t.list.relation2[i].text = response.data.t.list.relation2[i].systemConfigName;
            }
      ...
      validateInfo() {
        ...
        if (Utils.isEmpty(this.formData.linealMail.trim())) {
          Toast('联系人不能为空')
          return;
        }
        ...
        if (Utils.isEmpty(this.formData.linealMail2.trim())) {
          Toast('紧急联系人不能为空')
          return;
        }
        // if (!Utils.verifyInt(this.formData.linealMail.trim()) || this.formData.linealMail.trim().length != 11) {
        //   Toast("请输入正确的手机号")
        //   return;
        // }
        let params1 = {
        ...
        Base.requestCas(params1, '/validate/info').then((response) => {
